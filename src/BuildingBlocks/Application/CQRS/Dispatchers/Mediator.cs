...
public class Mediator : IMediator
{
        ...
        int capacity = 1000)
    {
        ...
        _ = Task.Run(() => ProcessCommands(_cts.Token));
        _ = Task.Run(() => ProcessStreams(_cts.Token));
    }
        ...
        CancellationToken ct = default)
    {
        ...
        await _commandChannel.Writer.WriteAsync(envelope, ct).ConfigureAwait(false);
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        await _streamChannel.Writer.WriteAsync(envelope, ct).ConfigureAwait(false);

        await foreach (var item in responseChannel.Reader.ReadAllAsync(ct).ConfigureAwait(false))
        {
    ...
    {
        await foreach (var envelope in _commandChannel.Reader.ReadAllAsync(ct).ConfigureAwait(false))
        {
            try
            {
                ...
                var result = await pipeline(envelope.Command, handler, ct).ConfigureAwait(false);
    ...
    {
        await foreach (var envelope in _streamChannel.Reader.ReadAllAsync(ct).ConfigureAwait(false))
        {
            try
            {
                ...
                await foreach (var item in pipeline(envelope.Message, handler, ct).ConfigureAwait(false))
                {
                    if (item?.GetType() == envelope.ResponseType)
                    {
                        await envelope.ResponseChannel.Writer.WriteAsync(item, ct).ConfigureAwait(false);
                    }
                }
            }
            catch (Exception ex)
    ...
    private MessageHandlerDelegate<IMessage, object> GetHandler(Type messageType, Type responseType)
    {
        ...
        return async (msg, ct) =>
        {
            ...
            return await (result is Task task
                ...
                : Task.FromResult(result!)).ConfigureAwait(false);
        };
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        await Task.WhenAll(
            ...
            _streamChannel.Reader.Completion).ConfigureAwait(false);
        _cts.Dispose();
    }
